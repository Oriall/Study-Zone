function thong_ke() {
    document.querySelector(".thong_ke").style.backgroundColor = '#1F1C2D';
    document.querySelector(".thong_ke").style.opacity = '0.9';
    document.querySelector(".theo_doi").style.backgroundColor = 'transparent';
    document.querySelector(".theo_doi").style.opacity = '1';
    document.querySelector(".tro_chuyen").style.backgroundColor = 'transparent';
    document.querySelector(".phong_chung").style.backgroundColor = 'transparent';
    document.querySelector(".phong_chung").style.opacity = '1';
    document.querySelector(".tro_chuyen").style.opacity = '1';
    document.querySelector(".thong_ke_right").style.display = "block";
    document.querySelector(".body_right").style.display = "none";
    document.querySelector(".end_right").style.display = "none";
    document.querySelector(".camera_box_bg").style.display = "none";
    document.querySelector(".phong_chung_right").style.display = "none";
    document.querySelector(".thong_ke_chung").style.display = "none";
    document.querySelector(".theo_doi_box").style.display = "none";
}

function tro_chuyen() {
    document.querySelector(".tro_chuyen").style.backgroundColor = '#1F1C2D';
    document.querySelector(".tro_chuyen").style.opacity = '0.9';
    document.querySelector(".theo_doi").style.backgroundColor = 'transparent';
    document.querySelector(".theo_doi").style.opacity = '1';
    document.querySelector(".thong_ke").style.backgroundColor = 'transparent';
    document.querySelector(".thong_ke").style.opacity = '1';
    document.querySelector(".phong_chung").style.backgroundColor = 'transparent';
    document.querySelector(".phong_chung").style.opacity = '1';
    document.querySelector(".thong_ke_right").style.display = "none";
    document.querySelector(".body_right").style.display = "flex";
    document.querySelector(".end_right").style.display = "flex";
    document.querySelector(".camera_box_bg").style.display = "none";
    document.querySelector(".phong_chung_right").style.display = "none";
    document.querySelector(".thong_ke_chung").style.display = "none";
    document.querySelector(".theo_doi_box").style.display = "none";
}
function phong_chung(){
    document.querySelector(".phong_chung").style.backgroundColor = '#1F1C2D';
    document.querySelector(".phong_chung").style.opacity = '0.9';
    document.querySelector(".theo_doi").style.backgroundColor = 'transparent';
    document.querySelector(".theo_doi").style.opacity = '1';
    document.querySelector(".tro_chuyen").style.backgroundColor = 'transparent';
    document.querySelector(".tro_chuyen").style.opacity = '1';
    document.querySelector(".thong_ke").style.backgroundColor = 'transparent';
    document.querySelector(".thong_ke").style.opacity = '1';
    document.querySelector(".thong_ke_right").style.display = "none";
    document.querySelector(".body_right").style.display = "none";
    document.querySelector(".end_right").style.display = "none";
    document.querySelector(".camera_box_bg").style.display = "none";
    document.querySelector(".thong_ke_chung").style.display = "block";
    document.querySelector(".phong_chung_right").style.display = "none";
    document.querySelector(".theo_doi_box").style.display = "none";

}
function phong_chung_rieng(){
    document.querySelector(".phong_chung").style.backgroundColor = 'transparent';
    document.querySelector(".phong_chung").style.opacity = '1';
    document.querySelector(".theo_doi").style.backgroundColor = 'transparent';
    document.querySelector(".theo_doi").style.opacity = '1';
    document.querySelector(".tro_chuyen").style.backgroundColor = 'transparent';
    document.querySelector(".tro_chuyen").style.opacity = '1';
    document.querySelector(".thong_ke").style.backgroundColor = 'transparent';
    document.querySelector(".thong_ke").style.opacity = '1';
    document.querySelector(".thong_ke_right").style.display = "none";
    document.querySelector(".body_right").style.display = "none";
    document.querySelector(".end_right").style.display = "none";
    document.querySelector(".camera_box_bg").style.display = "none";
    document.querySelector(".thong_ke_chung").style.display = "none";
    document.querySelector(".phong_chung_right").style.display = "block";
    document.querySelector(".theo_doi_box").style.display = "none";

}
function theo_doi(){
    document.querySelector(".theo_doi").style.backgroundColor = '#1F1C2D';
    document.querySelector(".theo_doi").style.opacity = '0.9';
    document.querySelector(".phong_chung").style.backgroundColor = 'transparent';
    document.querySelector(".phong_chung").style.opacity = '1';
    document.querySelector(".tro_chuyen").style.backgroundColor = 'transparent';
    document.querySelector(".tro_chuyen").style.opacity = '1';
    document.querySelector(".thong_ke").style.backgroundColor = 'transparent';
    document.querySelector(".thong_ke").style.opacity = '1';
    document.querySelector(".thong_ke_right").style.display = "none";
    document.querySelector(".body_right").style.display = "none";
    document.querySelector(".end_right").style.display = "none";
    document.querySelector(".camera_box_bg").style.display = "none";
    document.querySelector(".thong_ke_chung").style.display = "none";
    document.querySelector(".phong_chung_right").style.display = "none";
    document.querySelector(".theo_doi_box").style.display = "flex";

}
// function wr_22() {
//     document.querySelector('.video_tailieu_box').classList.remove('animation-status6');
//     document.querySelector('.video_tailieu_box').classList.add('animation-status7');
//     setTimeout(function () {
//         document.querySelector('.video_tailieu_box').style.display = "none";
//         document.getElementById('screen_box').classList.add('animation-status6');
//         document.getElementById('screen_box').style.display = 'flex'; // Display the div
//     }, 300);
// }
